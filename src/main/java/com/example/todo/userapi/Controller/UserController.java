...
public class UserController {
    ...
    @PutMapping("/promote")
    // 권한 검사 (해당 권한이 아니라면 인가 처리 거부-> 403코드 리턴)
    // 메서드 호출 전에 검사-> 요청 당시 토큰에 있는 유저 정보가
    // ROLE_COMMON이라는 권한을 가지고 있는지 검사.
    ...
    public  ResponseEntity<?> loadFile(
        ...
        try {
            ...
            if (!profileFile.exists()){
                // 모든 사용자가 프로필 사진을 가지는 것은 아니다. -> 프사가 없는 사람들은 경로가 없다
                // 만약 존재하지 않는 경로라면 클라이언트에게 404status를 리턴.
            ...
            MediaType contentType = FindExceptionAndGetMediaType(filePath);
            if (contentType == null){
                return ResponseEntity.internalServerError()
    ...
    private MediaType FindExceptionAndGetMediaType(String filePath) {

        // 파일 경로에서 확장자 추출하기
        // C:/todo_upload/머시깽머시깽.jpg
        String ext
    ...
    public ResponseEntity<?> kakaoLogin(String code){
        log.info("/api/auth/kakaoLigin - GET! -code: {}",code);
